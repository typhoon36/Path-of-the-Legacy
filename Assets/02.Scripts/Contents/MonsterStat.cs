...
public class MonsterStat : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject a_Obj;
        if (Managers.Data.Monster.TryGetValue(Id, out a_Obj) == true)
        {
            MonsterStat a_Stat = a_Obj.GetComponent<MonsterStat>();
            _name = a_Stat.Name;
            _hp = a_Stat.Hp;
            _maxHp = a_Stat.MaxHp;
            _attack = a_Stat.Attack;
            _dropExp = a_Stat.DropExp;
            _dropGold = a_Stat.DropGold;
            _dropItemId = a_Stat.DropItemId;
            _movespeed = a_Stat.MoveSpeed;
        }
    }

    // 공격을 받았을 때
    MonsterController _monster;
    public virtual void OnAttacked(int a_SkillAttack = 0)
    {
        // 일반 몬스터만 피격 상태 진행
        ...
        // 스킬 데미지 확인
        if (a_SkillAttack != 0)
            a_Damage = Mathf.Max(0, a_SkillAttack);
        else
            a_Damage = Mathf.Max(0, Managers.Game.Attack);
    ...
    // 피격 데미지 출력
    void HitEffect(int a_Damage)
    {
        // UI_HitEffect 생성 후 데미지 text 넣기
        UI_HitEffect a_HitObj = Managers.UI.MakeWorldSpaceUI<UI_HitEffect>(gameObject.transform);
        a_HitObj.m_HitText.text = a_Damage.ToString();

        // 생성 위치 설정
        float randomX = Random.Range(-0.5f, 0.5f);
        float valueY = GetComponent<Collider>().bounds.size.y * 0.8f;
        a_HitObj.transform.position = transform.position + new Vector3(randomX, valueY, 0);
    }
}
